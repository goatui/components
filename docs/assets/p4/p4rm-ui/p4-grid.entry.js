...
import { d as debounce } from './utils-7b05b85c.js';
...
const P4Grid = class {
  constructor(hostRef) {
    ...
    this.onSelectAllClick = () => {
      ...
      if (this.isSelectAll)
        selectedRows = this.data.map((row) => row[this.keyField]);
  ...
  renderBody() {
    ...
    this.data.forEach((row) => {
      ...
      const bodyRightRow = [];
      if (this.selectionType === 'checkbox')
        bodyLeftRow.push(h("div", { class: { 'col': true, 'col-hover': this.hoverRecord === row }, style: { width: CHECKBOX_WIDTH } }, h("p4-checkbox", { class: "checkbox", size: "sm", value: this.selectedRows.includes(row[this.keyField]), onP4Change: () => this.onRowSelectClick(row) })));
      this.columns.forEach((col) => {
        let colWidth = DEFAULT_CELL_WIDTH;
        if (col.width)
          colWidth = parseInt(col.width);
